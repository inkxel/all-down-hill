# ALL DOWN HILL — Knowledge Layer (AGENTS.md)

> **What ALL DOWN HILL is.** A 3D endless snowboarding game that ships as one self-contained `index.html` — Three.js from a CDN, every asset generated at runtime, deployed as a static file on GitHub Pages.

This is the **canonical** agent-instructions file for this knowledge layer, per the cross-tool [AGENTS.md](https://agents.md) standard — any agent (Claude Code, Cursor, Codex, Aider, …) reads it. The sibling `knowledge/CLAUDE.md` is a thin pointer back here; keep the orientation in this file, not duplicated there. Claude Code discovers this layer when a session opens `~/Code/personal/all-down-hill/knowledge/` (the `CLAUDE.md` pointer triggers progressive loading, which leads here). Read it before doing architectural work. The wiki below documents how ALL DOWN HILL is built, why it's shaped that way, and what's still open.
...
- **`knowledge/`** (this layer) — *build history.* Why the game is shaped the way it is, what was tried and rejected, what broke and why. Read this before changing physics, terrain generation, or the render pipeline.
- **`README.md`** (repo root) — *what the game is and how to play it.* Controls, live URL, stack. Deliberately short; it is not a design doc.
- **`index.html`** — *the entire program.* There is no `src/`, no build step, no module graph. The file is heavily sectioned with banner comments; the code-map below indexes it.

There is no external runtime knowledge source — the game reads nothing at run time except the Three.js CDN module.
...
## README & research conventions (the CoSi standard)

- **The README is WHAT / WHY / DIRECTION / NAVIGATION only.** It's the canonical one-page outline — what this is, why it exists, where it's going, how to navigate. Write it from the standard structure in `~/.claude/skills/knowledge-layer/references/readme-template.md` so every CoSi repo is consistent.
...
- **No meeting origins, no client/internal-people names in the README** — keep repos client-neutral; that context lives in the broader CoSi/FOUNDRY layer.
...
### 2026-07-27 — Knowledge layer created
Scaffolded with the `knowledge-layer` skill. {{SEED_NOTE — what it was seeded with, e.g. "Seeded with N decision records and a backfill journal entry from the project's current state."}}
